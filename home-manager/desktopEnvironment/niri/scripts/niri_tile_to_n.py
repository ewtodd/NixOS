...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Only apply tiling logic on workspaces belonging to this output (e.g. HDMI-A-1). Default: all outputs.",
)
    ...
    for evt_name, evt_data in niri_reader.read_eventstream():
        ...
        if closed_window_data is not None:
            if MAXIMIZE_SOLOS_ON_CLOSE:
                ...
                if TARGET_OUTPUT is None or wspace_state.get(curr_wspace_id, {}).get("output") == TARGET_OUTPUT:
        ...
        if newest_window_data is not None:
            ...
            # Ignore newly created floating windows
            if newest_window_data["is_floating"]:
                continue

            curr_wspace_id = newest_window_data["workspace_id"]

            # Restrict tiling to a single output, if requested
            if TARGET_OUTPUT is not None and wspace_state.get(curr_wspace_id, {}).get("output") != TARGET_OUTPUT:
                continue

            # Outside of --output mode, also skip windows the upstream heuristic flagged as
            # already-maximized (assumed to be user-rule driven). In --output mode the heuristic
            # is unreliable on portrait panels, so we ignore it.
            if TARGET_OUTPUT is None and newest_window_data["is_maximized"]:
                continue
            ...
            # When --output is used, fill columns in groups of TILE_TO_N: if the new window
            # opened to the right of a column that has fewer than TILE_TO_N rows, consume it
            # into that column. Otherwise leave it alone (start a fresh column). This loops
            # indefinitely so windows pack as pairs/triples/etc. across the workspace.
            if TARGET_OUTPUT is not None:
                if num_tile_wins == 0:
                    continue
                new_col_idx = newest_window_data.get("col_idx")
                if new_col_idx is not None and new_col_idx > 1:
                    left_col_count = sum(
                        1 for w in curr_tile_wins.values() if w.get("col_idx") == new_col_idx - 1
                    )
                    if left_col_count < TILE_TO_N:
            ...
            # Apply landscape (side-by-side-first) tiling
